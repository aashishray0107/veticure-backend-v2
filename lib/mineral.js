export function validateMinerals(
  ...
  try {
    ...
    if (!chicken || !rice || !egg) {
      throw new Error("Base ingredients missing in database");
    }

    if (!calciumSupplement) {
      throw new Error("Calcium supplement missing in database");
    }
    ...
    if (total_calcium < required_calcium) {
      ...
      if (calciumPerGramSupplement <= 0) {
        throw new Error("Invalid supplement calcium value");
      }

      calcium_supplement_g =
        deficit / calciumPerGramSupplement;

      // Hard safety cap (15g/day max supplement)
  ...
  Number(((total_calcium * 1000) / final_calories).toFixed(3));
    ...
    return {
      ...
      regulatory_reference:
        life_stage === "Puppy"
          ? "NRC 2006 Large Breed Growth (3.0–4.5 g Ca/1000 kcal, Ca:P 1.1–1.4)"
