export function validateMinerals(
  ...
  try {
    ...
    // =============================
    // WORKING COPY (DO NOT MUTATE INPUT)
    // =============================
    ...
    // =============================
    // MINERAL CALCULATION FUNCTION
    // =============================

    const computeMinerals = () => {
      let total_calcium =
      ...
      return { total_calcium, total_phosphorus };
    };

    let { total_calcium, total_phosphorus } =
      computeMinerals();
    ...
    // =============================
    // SENIOR AUTO PHOSPHORUS REDUCTION
    // =============================
    ...
    if (life_stage === "Senior") {
      ...
      while (
        phosphorus_per_1000_kcal > 0.5 &&
        iterations < 10
      ) {
        if (egg_g > 20) {
          egg_g -= 5;
        } else if (chicken_g > 200) {
          chicken_g -= 10;
        } else {
          break;
        }
        ...
        phosphorus_per_1000_kcal = Number(
          (
            (total_phosphorus * 1000) /
            final_calories
          ).toFixed(3)
        );
      ...
      if (phosphorus_per_1000_kcal > 0.5) {
        ...
        renal_warning =
          "Unable to reduce phosphorus below senior threshold safely.";
      } else {
        adjustment_note =
          "Protein sources reduced automatically to meet senior phosphorus threshold.";
      }
    }
    ...
    return {
      ...
      calcium_supplement_g: Number(
        calcium_supplement_g.toFixed(2)
      ),
      lifecycle_compliant,
      renal_compliant,
      renal_warning,
