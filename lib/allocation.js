if (!engineData?.Expanded_Food_Composition_Database_v2?.Ingredients) {
  throw new Error("Expanded_Food_Composition_Database_v2 missing");
}
...
const pool = engineData?.Rotational_Ingredient_Pools?.[poolKey];

if (!pool) {
  throw new Error(`No rotation pool for lifecycle: ${poolKey}`);
}
...
if (!foodDB) {
  ...
  if (!foodDB || Object.keys(foodDB).length === 0) {
  throw new Error("Food database empty or invalid");
}
