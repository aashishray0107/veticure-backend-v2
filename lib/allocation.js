import fs from "fs";
import path from "path";

export function allocateIngredients(macroReport) {

  try {
    // 🔹 Load JSON safely
    const dataPath = path.join(process.cwd(), "data", "labrador_engine.json");
    const rawData = fs.readFileSync(dataPath, "utf-8");
    const engineData = JSON.parse(rawData);

    const foodDB = engineData.Food_Composition_Database;

    if (!foodDB) {
      return { error: "Food database missing" };
    }

    const chicken = foodDB.Boiled_Chicken_Breast_Skinless;
    const rice = foodDB.Boiled_White_Rice;

    if (!chicken || !rice) {
      return { error: "Required ingredients missing in database" };
    }

    // 🔹 Convert protein grams → chicken grams
    const chicken_grams =
      macroReport.protein_g / (chicken.protein_g_per_100g / 100);

    // 🔹 Convert carbs grams → rice grams
    const rice_grams =
      macroReport.carbs_g / (rice.carbs_g_per_100g / 100);

    return {
      chicken_g: Math.round(chicken_grams),
      rice_g: Math.round(rice_grams)
    };

  } catch (err) {
    return {
      error: "Allocation engine failed",
      details: err.message
    };
  }
}