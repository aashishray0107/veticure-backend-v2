export function evaluateLifecycle(
  ...
  // 🔥 JSON-Driven Deviation Category Mapping
  ...
  // 🔥 Convert Deviation Category → Numeric BCS (from JSON)
  const estimateMap =
    engineData?.BCS_Automatic_Detection_Logic
  ...
  const bcsRange =
    estimateMap[deviationCategory].BCS_Range;

  const deviationBCS =
    (bcsRange[0] + bcsRange[1]) / 2;
